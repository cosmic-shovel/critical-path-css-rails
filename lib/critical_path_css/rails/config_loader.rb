module CriticalPathCss
  module Rails
    class ConfigLoader
      ...
      def config
        @config ||= YAML.safe_load(ERB.new(File.read(configuration_file_path)).result, permitted_classes: [], permitted_symbols: [], aliases: true)[::Rails.env]
      end
      ...
      def format_css_paths
        ...
        unless config['css_paths']
          config['css_paths'] = [ActionController::Base.helpers.stylesheet_path(config['manifest_name'], host: "#{::Rails.root}/public")]
        end
      end
